...
' ======================================================================================
' サービス記録自動入力マクロ (VBA版 - Mac完全対応修正v15)
'
' 変更点 (Mac対応v15):
...
Sub ProcessAllSheets()
    ...
    Dim writtenSheetNames As String
    ...
    For Each ws In wb.Worksheets
        If ws.Name <> "Master" Then
            ...
            If ProcessSheet(ws, masterData) Then
                writtenCount = writtenCount + 1
                writtenSheetNames = writtenSheetNames & ws.Name & vbCrLf
            End If
    ...
    If writtenSheetNames <> "" Then writtenSheetNames = Left(writtenSheetNames, Len(writtenSheetNames) - 1)
    MsgBox processedCount & " 枚のシート処理が完了しました。" & vbCrLf & vbCrLf & "記入を行ったシート数: " & writtenCount & vbCrLf & vbCrLf & "記入を行ったシート名:" & vbCrLf & IIf(writtenSheetNames = "", "（なし）", writtenSheetNames), vbInformation
End Sub
...
Function ProcessSheet(ws As Worksheet, ByRef masterData() As MasterRecord) As Boolean
    ...
    Dim newWriteR() As Boolean, newWriteU() As Boolean  ' マクロが新規にコードを算出して書き込んだ行か
    ReDim calcR(1 To numRows)
    ReDim calcU(1 To numRows)
    ReDim newWriteR(1 To numRows)
    ReDim newWriteU(1 To numRows)
    ...
    ' --- 1. グルーピング ---
    Dim fVal As String  ' F列（A or F）。A+F の組み合わせの場合は別グループにする
    For i = 1 To numRows
        rDate = inputData(i, 1)
        rStart = inputData(i, 10)   ' J列
        rEnd = inputData(i, 11)     ' K列
        helperCount = ParseHelperCount(CStr(inputData(i, 13))) ' M列
        fVal = VBA.Trim(VBA.UCase(CStr(inputData(i, 6))))  ' F列
        If fVal <> "A" And fVal <> "F" Then fVal = ""
        ...
        rowArr(5) = isOverlap: rowArr(6) = fVal
        ...
        Else
            ...
            Else
                ...
                ' 同一日・119分以内でも、F列が A と F の別々の組み合わせの場合は別グループ
                Dim prevF As String: prevF = CStr(lastArr(6))
                Dim splitByAF As Boolean: splitByAF = False
                If prevF <> "" And fVal <> "" And prevF <> fVal Then splitByAF = True

                If diffMinutes <= 119 And diffMinutes > -1 And Not splitByAF Then
                    currentGroup.Add rowArr
                Else
                    groups.Add currentGroup
                    Set currentGroup = New Collection
...
NextRow:
    ...
    ' --- 2. 計算 & マッチング ---
    Dim g As Variant, prevG As Variant, item As Variant
    Dim lastItemIdx As Long, prevLastItemIdx As Long
    Dim gi As Long

    For gi = 1 To groups.Count
        Set g = groups(gi)
        Dim rawBuckets As TimeBucket, finalBuckets As TimeBucket
        ...
        ' A+F 組み合わせ: 前グループが1行・当グループが1行・同一日・119分以内・F列がAとFで別 → 時間は別々に丸めて合算し、コードは当グループの行(17行目)にのみ出力
        Dim useAFCombined As Boolean: useAFCombined = False
        If gi >= 2 Then
            Set prevG = groups(gi - 1)
            If g.Count = 1 And prevG.Count = 1 Then
                Dim prevItem1 As Variant: prevItem1 = prevG.Item(1)
                Dim prevEndTime As Date: prevEndTime = CDate(prevItem1(3))
                Dim currStart As Date: currStart = CDate(firstItem(2))
                Dim prevFVal As String: prevFVal = CStr(prevItem1(6))
                Dim currF As String: currF = CStr(firstItem(6))
                If IsSameDay(prevEndTime, currStart) Then
                    If DateDiff("n", prevEndTime, currStart) <= 119 And DateDiff("n", prevEndTime, currStart) >= 0 Then
                        If prevFVal <> "" And currF <> "" And prevFVal <> currF Then useAFCombined = True
                    End If
                End If
            End If
        End If

        If useAFCombined Then
            Dim rawPrev As TimeBucket
            rawPrev.Early = 0: rawPrev.Day = 0: rawPrev.Night = 0: rawPrev.Deep = 0
            For Each item In prevG
                b = CalculateBuckets(CDate(item(2)), CDate(item(3)))
                rawPrev.Early = rawPrev.Early + b.Early: rawPrev.Day = rawPrev.Day + b.Day
                rawPrev.Night = rawPrev.Night + b.Night: rawPrev.Deep = rawPrev.Deep + b.Deep
                prevLastItemIdx = item(0)
            Next item
            Dim finalPrev As TimeBucket: finalPrev = RoundRawToFinalBuckets(rawPrev)
            Dim finalCurr As TimeBucket: finalCurr = RoundRawToFinalBuckets(rawBuckets)
            finalBuckets.Early = finalPrev.Early + finalCurr.Early
            finalBuckets.Day = finalPrev.Day + finalCurr.Day
            finalBuckets.Night = finalPrev.Night + finalCurr.Night
            finalBuckets.Deep = finalPrev.Deep + finalCurr.Deep
            ' 前グループの行にはコードを出さない
            calcR(prevLastItemIdx) = "": calcU(prevLastItemIdx) = ""
        Else
            Dim totalRaw As Double
            totalRaw = rawBuckets.Early + rawBuckets.Day + rawBuckets.Night + rawBuckets.Deep
            If totalRaw > 0 Then
                finalBuckets = RoundRawToFinalBuckets(rawBuckets)
            Else
        ...
        totalFinal = finalBuckets.Early + finalBuckets.Day + finalBuckets.Night + finalBuckets.Deep
        ...
        ' 結果の書き込みを行ったのは「既存値でなくマクロが算出したコード」を書いた行のみ
        If existingR = "" And proposedR <> "" And Left(proposedR, 1) <> "(" Then newWriteR(lastItemIdx) = True
        If existingU = "" And proposedU <> "" And Left(proposedU, 1) <> "(" Then newWriteU(lastItemIdx) = True
    Next gi
    ...
    For i = 1 To numRows
        ...
        If newWriteR(i) Or newWriteU(i) Then hadEntry = True
...
' 生の時間帯集計を30分単位に丸め、最終的な時間帯配分（0.5単位）を返す。A+F合算時に使用。
Private Function RoundRawToFinalBuckets(raw As TimeBucket) As TimeBucket
    Dim totalRaw As Double
    Dim roundedUnits As Long
    Dim finalBuckets As TimeBucket, remBuckets As TimeBucket
    Dim u As Long, maxVal As Double, maxKey As String

    totalRaw = raw.Early + raw.Day + raw.Night + raw.Deep
    If totalRaw <= 0 Then
        RoundRawToFinalBuckets.Early = 0: RoundRawToFinalBuckets.Day = 0
        RoundRawToFinalBuckets.Night = 0: RoundRawToFinalBuckets.Deep = 0
        Exit Function
    End If
    ...
    For u = 1 To roundedUnits
        ...
        End Select
    Next u
    RoundRawToFinalBuckets = finalBuckets
End Function
