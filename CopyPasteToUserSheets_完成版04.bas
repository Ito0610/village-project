...
' ・時間重複の黄: B列同一利用者 & F列同日 & C列開始〜D列終了がかぶる場合のみ C〜D を黄（終了=開始は除く）
...
Public Sub CopyPasteToUserSheets()
    ...
    '  (1) B列が同じ利用者  (2) F列の日付が同じ  (3) C列開始〜D列終了の時間帯がかぶる（終了=開始は重複としない）
    ...
    If lastRow >= 2 Then
        ...
        For i = 1 To numOta
            ...
            For j = 1 To numOta
                ...
                ' (2) F列の日付が同じ日か（別の日なら時間がかぶっても重複にしない）
                If Not IsSameDate(wsCopy.Cells(r, 6).Value, wsCopy.Cells(r2, 6).Value) Then GoTo NextJ2
