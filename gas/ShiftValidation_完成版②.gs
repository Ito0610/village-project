 ...
 * 行の増減で位置が変わっても対応できる。
 *
 * 完成版②：他GASとの共存（addShiftValidationMenu）、「名前を黒色に戻す」は対象7列（D,J,P,V,AB,AH,AN）のブロック内のみ。
 */
 ...
 */
function resetAllHelperNamesToNormal() {
  ...
  if (blocks.length === 0) {
    SpreadsheetApp.getUi().alert('シフト表・シフト希望のブロックを特定できません。');
    return;
  }
