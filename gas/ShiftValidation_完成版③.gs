/**
 * シフト表・シフト希望 照合チェック（Google Apps Script）【完成版③】
 ...
 * 行の増減で位置が変わっても対応できる。
 *
 * 【完成版③】スマホ対応：シフトチェックシートのB2でコマンド入力起動、G3で照合対象シート名を指定。
 */
...
/** 「名前を黒色に戻す」で対象とする列のみ（D, J, P, V, AB, AH, AN）。表の他の列は触れない。 */
var RESET_HELPER_COLUMNS = [4, 10, 16, 22, 28, 34, 40];

/**
 * セル入力でスクリプトを起動する設定（スマホなどメニューが出ない環境用）。
 * 指定セルに以下のいずれかを入力すると、編集確定で対応する処理が実行されます。
 * TRIGGER_SHEET_NAME: トリガーを反応させるシート名（B2 があるシート）。
 * DATA_SHEET_NAME_CELL: 照合対象シート名を読み取るセル（トリガーシート上の「行, 列」1始まり）。
 *   例: [1, 1] = A1。ここに「3月」など対象シート名を書いておけば毎月スクリプトを編集せずに済む。
 *   ［セルが空のとき］DATA_SHEET_NAME を参照（未設定ならエラー）。
 * TRIGGER_ROW / TRIGGER_COLUMN: 起動対象のセル（1始まり）。例: 2,2 = B2。
 */
var EDIT_TRIGGER = {
  TRIGGER_SHEET_NAME: 'シフトチェック',
  /** 照合対象シート名を読み取るセル [行, 列]（1始まり）。例: [3, 7] = G3 */
  DATA_SHEET_NAME_CELL: [3, 7],  // G3 に「3月」など対象シート名を記載
  /** 上記セルが空のときのフォールバック。通常はセルで管理するため null でよい */
  DATA_SHEET_NAME: null,
  TRIGGER_ROW: 2,
  TRIGGER_COLUMN: 2,  // B2
  /** 入力値 → 曜日インデックス(0-6) または 'reset'（名前を黒色に戻す） */
  COMMANDS: {
    '0': 0, '日': 0, '日曜': 0,
    '1': 1, '月': 1, '月曜': 1,
    '2': 2, '火': 2, '火曜': 2,
    ...
    '5': 5, '金': 5, '金曜': 5,
    '6': 6, '土': 6, '土曜': 6,
    '黒': 'reset', 'リセット': 'reset', 'reset': 'reset'
  }
};
 ...
 * 他のGASと共存する場合：プロジェクト内の onOpen() から addShiftValidationMenu() を呼び出す。
 */
function addShiftValidationMenu() {
  ...
  menu.addItem('名前を黒色に戻す', 'resetAllHelperNamesToNormal');
  menu.addSeparator()
...
/**
 * トリガーシート上の「照合対象シート名」セルを読み取る。
 * @param {GoogleAppsScript.Spreadsheet.Sheet} triggerSheet シフトチェックシート
 * @return {string} シート名（空白時は ''）
 */
function getDataSheetNameFromCell(triggerSheet) {
  var cell = EDIT_TRIGGER.DATA_SHEET_NAME_CELL;
...
/**
 * 指定セルにコマンド文字が入力されたら照合スクリプトを起動する（編集時トリガー）。
 * スマホではメニューが出ないため、このセル入力を利用してください。
 * 入力例: 日 / 月 / 火 / 水 / 木 / 金 / 土、または 0〜6、または 黒 / リセット
 */
function onEdit(e) {
  if (!e || !e.range) return;
  var sheet = e.range.getSheet();
  if (EDIT_TRIGGER.TRIGGER_SHEET_NAME && sheet.getName() !== EDIT_TRIGGER.TRIGGER_SHEET_NAME) return;
  if (e.range.getRow() !== EDIT_TRIGGER.TRIGGER_ROW || e.range.getColumn() !== EDIT_TRIGGER.TRIGGER_COLUMN) return;

  var val = (e.value != null ? String(e.value) : '').trim();
  if (!val) return;

  var cmd = EDIT_TRIGGER.COMMANDS[val];
  if (cmd === undefined) return;

  var dataSheet = getDataSheetNameFromCell(sheet) || EDIT_TRIGGER.DATA_SHEET_NAME || undefined;
  if (!dataSheet) {
    var cellDesc = EDIT_TRIGGER.DATA_SHEET_NAME_CELL && EDIT_TRIGGER.DATA_SHEET_NAME_CELL.length >= 2
      ? '行' + EDIT_TRIGGER.DATA_SHEET_NAME_CELL[0] + '・列' + EDIT_TRIGGER.DATA_SHEET_NAME_CELL[1]
      : 'A1';
    e.range.setValue('エラー: 照合対象のシート名を' + cellDesc + 'のセルに入力してください。');
    return;
  }

  try {
    if (cmd === 'reset') {
      resetAllHelperNamesToNormal(dataSheet);
      e.range.setValue('実行しました（黒に戻す）');
    } else {
      runShiftValidationForColumn(cmd, dataSheet);
      e.range.setValue('実行しました（' + DAY_LABELS[cmd] + '）');
    }
  } catch (err) {
...
/**
 * 照合対象のシートを返す。optSheetName が指定されていればその名前のシート、なければアクティブシート。
 * @param {string|undefined} optSheetName シート名（省略可）
 * @return {GoogleAppsScript.Spreadsheet.Sheet}
 */
function getSheetForValidation(optSheetName) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  if (optSheetName) {
    var target = ss.getSheetByName(optSheetName);
    if (!target) throw new Error('照合用シート「' + optSheetName + '」が見つかりません。DATA_SHEET_NAME を確認してください。');
    return target;
  }
...
/**
 * 対象列（D, J, P, V, AB, AH, AN）のブロック内のみ黒色・標準文字に戻す。
 * 表の他の列は一切触れない。メニュー「名前を黒色に戻す」から実行する。
 * @param {string|undefined} optSheetName 照合対象シート名（セル入力起動時は EDIT_TRIGGER.DATA_SHEET_NAME を渡す）
 */
function resetAllHelperNamesToNormal(optSheetName) {
  var sheet = getSheetForValidation(optSheetName);
  var markerRows = getMarkerRowsByColor(sheet);
  if (!markerRows || Object.keys(markerRows).length === 0) {
    var msg = '対象色で塗ったマーカー行が検出できません。CONFIG の色コードと、シートのマーカー列（D列）の色を確認してください。';
  ...
  if (blocks.length === 0) {
    var msg2 = 'シフト表・シフト希望のブロックを特定できません。';
  ...
  for (var b = 0; b < blocks.length; b++) {
    ...
    // 対象7列（D, J, P, V, AB, AH, AN）のみ。ブロック行範囲内だけ処理する。
    // getRange(開始行, 開始列, 行数, 列数) なので列数は必ず 1
    for (var i = 0; i < RESET_HELPER_COLUMNS.length; i++) {
      var col = RESET_HELPER_COLUMNS[i];
      if (block.shiftTableStart <= block.shiftTableEnd) {
        var numTableRows = block.shiftTableEnd - block.shiftTableStart + 1;
        var tableRange = sheet.getRange(block.shiftTableStart, col, numTableRows, 1);
        tableRange.setFontColor(NORMAL_STYLE.foreground).setFontWeight(NORMAL_STYLE.bold ? 'bold' : 'normal');
      }
      if (hopeStart <= hopeEnd) {
        var numHopeRows = hopeEnd - hopeStart + 1;
        var hopeRange = sheet.getRange(hopeStart, col, numHopeRows, 1);
        hopeRange.setFontColor(NORMAL_STYLE.foreground).setFontWeight(NORMAL_STYLE.bold ? 'bold' : 'normal');
      }
    }
  }

  SpreadsheetApp.getUi().alert('対象列（D, J, P, V, AB, AH, AN）のブロック内を黒色・標準文字に戻しました。');
}
 ...
 * @param {string|undefined} optSheetName 照合対象シート名（セル入力起動時は EDIT_TRIGGER.DATA_SHEET_NAME を渡す）
 */
function runShiftValidationForColumn(columnIndex, optSheetName) {
  ...
  var blocks = buildRowBlocks(markerRows);
  if (blocks.length === 0) {
    var msg2 = 'シフト表・シフト希望のブロックを特定できません。対象色1〜6が、色1・色1・色2・色2…の順で並んでいるか確認してください。';
    SpreadsheetApp.getUi().alert(msg2);
    throw new Error(msg2);
  }
